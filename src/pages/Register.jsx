...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/customer/register`, {
        ...
        body: JSON.stringify({ name, email, password, password_confirmation: passwordConfirmation || password }),
      });
  ...
  const inputClass = "w-full rounded-xl border border-[var(--border)] bg-[var(--bg)] py-3 pl-10 pr-4 text-sm text-[var(--text)] outline-none placeholder:text-[var(--muted)] focus:border-violet-500 focus:ring-2 focus:ring-violet-100";
    ...
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      <header className="border-b border-[var(--border)] bg-[var(--card)] px-4 py-4 lg:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <p className="text-sm text-[var(--muted)]">
            ...
            <Link to="/account" className="font-semibold text-violet-600 hover:text-violet-700">Sign in</Link>
          </p>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-2xl px-4 py-10 lg:px-8">
        ...
        <div className="rounded-3xl bg-[var(--card)] p-8 shadow-sm">
          <h1 className="text-2xl font-extrabold text-[var(--text)]">Create Account ✨</h1>
          <p className="mt-1 text-sm text-[var(--muted)]">Join Marketify and start your shopping journey</p>
          ...
          <form onSubmit={handleRegister} className="mt-8 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? "text" : "password"} placeholder="Create a password" value={password} onChange={(e) => setPassword(e.target.value)} className={`${inputClass} pr-10`} />
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showConfirm ? "text" : "password"} placeholder="Confirm your password" value={passwordConfirmation} onChange={(e) => setPasswordConfirmation(e.target.value)} className={`${inputClass} pr-10`} />
                <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-3 top-3 text-[var(--muted)] hover:text-violet-600">
            ...
            <label className="flex items-start gap-2 text-sm text-[var(--muted)]">
              ...
              I agree to the{" "}
              <span>
                <Link to="/" className="font-medium text-violet-600">Terms & Conditions</Link>{" "}
                and{" "}
                <Link to="/" className="font-medium text-violet-600">Privacy Policy</Link>
              </span>
            </label>
